val input4 = """bkwzkqsxq-tovvilokx-nozvyiwoxd-172[fstek]
...
val ABC = listOf<Char>('a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z')

inner class RoomFilter {
    fun sumRealRooms(input: String): Int {
        return getRealRooms(input)
            .sumBy { it.filter { it in "1234567890" }.toInt() }
    }

    fun findNorthPoleRoom(input: String): Int {
        return getRealRooms(input)
            .map { decodeRoomName(it) }
            .filter { it.contains("north") }
            .filter { it.contains("pole") }
            .map { it.filter { it in "1234567890" }.toInt() }
            .first()
    }

    private fun decodeRoomName(roomLine: String): String {
        val sum = roomLine.filter { it in "1234567890" }.toInt()
        val (left, _) = roomLine.split("[")
        ...
        for (letter in left) {
            ...
            val i = ABC.indexOf(letter)
            val newI = (i + sum) % ABC.size
            val newLetter = ABC[newI]
            rotatedName += newLetter
        }

        return rotatedName + sum
    }

    private fun getRealRooms(input: String): List<String> {
        return input
                .split("\n")
                .filter { check(it) }
    }

    private fun check(roomLine: String): Boolean {
        val (left, right) = roomLine
                .split("[")
                .map { it.filter { it in "qweasdzxcrtyfghvbnuiojklmp" } }
        ...
        val lettersByCounters = mutableMapOf<Int, MutableList<Char>>()
        for ((k, v) in counters) {
            lettersByCounters
                    .getOrPut(v) { mutableListOf<Char>() }
                    .add(k)
        }

        val counterPairs = mutableListOf<MyPair<Int, MutableList<Char>>>()
        for ((intK, charList) in lettersByCounters) {
            counterPairs.add(MyPair(intK, charList))
        }

        val sorted = counterPairs
                .sortedByDescending { it.a }
                .map { MyPair(it.a, it.b.sorted()) }
                .flatMap { it.b }
                .slice(0..4)
                .fold("") { str, letter -> str + letter.toString() }

        return sorted == right
    }
}

data class MyPair<T, K>(val a: T, val b: K)

val t = RoomFilter()
println("Part One")
println(t.sumRealRooms(input4))
println("Part Two")
println(t.findNorthPoleRoom(input4))
