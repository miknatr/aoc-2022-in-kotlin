class ChipFactory {
    val outputs = OutputStorage()
    val bots = BotTeam()

    fun botsCompareChips(input: String, chipsToWatch: Set<Int>): Int {
        val watcher = doWork(chipsToWatch, input)

        return watcher.watchedBots.first()
    }

    fun multipleOutputs(input: String, chipsToWatch: Set<Int>): Int {
        val watcher = doWork(chipsToWatch, input)

        return outputs.outputs[0]!!.first() * outputs.outputs[1]!!.first() * outputs.outputs[2]!!.first()
    }

    private fun doWork(chipsToWatch: Set<Int>, input: String): BotActWatcher {
        val watcher = BotActWatcher(chipsToWatch)

        val inputRegex = Regex("""^value (\d+) goes to bot (\d+)$""")
        val outputRegex = Regex("""^bot (\d+) gives low to (bot|output) (\d+) and high to (bot|output) (\d+)$""")
        for (command in input.split("\n")) {
            when {
                inputRegex.matches(command) -> {
                    val groups = inputRegex.matchEntire(command)!!.groups
                    bots
                            .getBot(groups[2]!!.value.toInt())
                            .addChip(groups[1]!!.value.toInt())
                }
                outputRegex.matches(command) -> {
                    val groups = outputRegex.matchEntire(command)!!.groups
                    bots
                            .getBot(groups[1]!!.value.toInt())
                            .setRule(groups[2]!!.value, groups[3]!!.value.toInt(), groups[4]!!.value, groups[5]!!.value.toInt())
                }
            }
        }

        while (bots.canAct()) {
            for ((_, bot) in bots.bots) {
                if (bot.canAct()) {
                    bot.act(bots, outputs, watcher)
                }
            }
        }
        return watcher
...
class OutputStorage {
    ...
    fun addToOutput(output: Int, chip: Int) {
        if (!outputs.containsKey(output)) {
            outputs[output] = mutableListOf()
        }

        outputs[output]!!.add(chip)
    }
}
...
class BotTeam {
    ...
    fun canAct(): Boolean = bots.any { it.value.canAct() }
}

class Bot(val num: Int) {
    private val chips = mutableListOf<Int>()

    private var outLowType: String? = null
    private var outLowNumber: Int? = null
    private var outHighType: String? = null
    private var outHighNumber: Int? = null

    fun setRule(outLowTypeIn: String, outLowNumberIn: Int, outHighTypeIn: String, outHighNumberIn: Int) {
        outLowType = outLowTypeIn
        outLowNumber = outLowNumberIn
        outHighType = outHighTypeIn
        outHighNumber = outHighNumberIn
    }
    fun addChip(chip: Int) = chips.add(chip)
    fun canAct(): Boolean = chips.size == 2
    fun act(team: BotTeam, outputs: OutputStorage, watcher: BotActWatcher) {
        if (!canAct()) {
            return
        }

        val lowChip = chips.min()!!
        val highChip = chips.max()!!
        giveChipTo(outLowType!!, outLowNumber!!, lowChip, team, outputs, watcher)
        giveChipTo(outHighType!!, outHighNumber!!, highChip, team, outputs, watcher)

        watcher.watch(num, lowChip, highChip)

        chips.clear()
    }

    private fun giveChipTo(type: String, number: Int, chip: Int, team: BotTeam, outputs: OutputStorage, watcher: BotActWatcher) {
        when (type) {
            "bot" -> {
                val bot = team.getBot(number)
                if (bot.canAct()) {
                    bot.act(team, outputs, watcher)
                }
                bot.addChip(chip)
            }
            "output" -> outputs.addToOutput(number, chip)
        }
    }
}

class BotActWatcher(val chipsToWatch: Set<Int>) {
    val watchedBots = mutableSetOf<Int>()

    fun watch(botNum: Int, chip1: Int, chip2: Int) {
        if (chip1 in chipsToWatch && chip2 in chipsToWatch) {
            watchedBots.add(botNum)
        }
    }
}

val input10 = """bot 127 gives low to output 1 and high to bot 180
...
val t = ChipFactory()
println("Part One")
println(t.botsCompareChips(input10, setOf(17, 61)))
println("Part Two")
println(t.multipleOutputs(input10, setOf(17, 61)))
