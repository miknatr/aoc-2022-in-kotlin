...
data class Prog(val name: String, val weight: Int, val childNames: List<String>, val children: MutableList<Prog> = mutableListOf(), val parents: MutableList<Prog> = mutableListOf()) {
    fun childrenWeight(): Int = children.sumBy { it.weight + it.childrenWeight() }
    fun fullWeight(): Int = weight + children.sumBy { it.weight + it.childrenWeight() }
}

fun parseLine(line: String): Prog {
    var nameAndWeight = line
    var childsStr = ""
    if (line.contains(" -> ")) {
        val explode = line.split(" -> ")
        nameAndWeight = explode[0]
        childsStr = explode[1]
    }

    val explode = nameAndWeight.split(" (")
    val name = explode[0]
    val weight = explode[1].trimEnd(')').toInt()

    val childNames = childsStr.split(", ").filter { it != "" }

    return Prog(name, weight, childNames)
}

val progs = input.split("\n").map { parseLine(it) }.map { it.name to it }.toMap()

for ((parentName, parent) in progs) {
    for (childName in parent.childNames) {
        progs[parentName]!!.children.add(progs[childName]!!)
        progs[childName]!!.parents.add(progs[parentName]!!)
    }
}

val rootProg = progs.toList().map { it.second }.filter { it.parents.size == 0 }.first()

println(rootProg.name)

fun printChilds(children: List<Prog>) {
    for (child in children) {
        println("${child.name}: ${child.weight} + ${child.childrenWeight()} = ${child.weight + child.childrenWeight()}")
    }
}

// simple manual iterative way:
// println()
// printChilds(rootProg.children)
// println()
// printChilds(progs["rilyl"]!!.children)
// println()
// printChilds(progs["aurik"]!!.children)
// println()
// printChilds(progs["jriph"]!!.children)
// println()
// println(2097 - 104)
// println()

var correctedWeight = 0
var currentRoot = rootProg
while (true) {
    val weightByProgs = currentRoot.children.groupBy { it.weight + it.childrenWeight() }

    val rightWeight = weightByProgs.filter { it.value.size != 1 }.map { it.value.first() }.first().fullWeight()

    val wrongProg = weightByProgs.filter { it.value.size == 1 }.map { it.value.first() }.first()

    val weightByProgsChild = wrongProg.children.groupBy { it.weight + it.childrenWeight() }

    if (weightByProgsChild.size == 1 || weightByProgsChild.size == 0) {
        correctedWeight = rightWeight - wrongProg.childrenWeight()
        break
    }

    currentRoot = wrongProg
}

println(correctedWeight)
