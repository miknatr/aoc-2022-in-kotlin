...
fun execCommands(input: String, registers: MutableMap<String, Int>): Int {
    var maxInProcess = 0
    ...
    for (command in commands) {
        val (changeReg, action, changeValueStr, compareReg, operator, compareValueStr) = regex.matchEntire(command)!!.destructured

        val changeValue = if (action == "inc") changeValueStr.toInt() else -changeValueStr.toInt()
        val compareValue = compareValueStr.toInt()

        if (!registers.containsKey(changeReg)) {
            registers[changeReg] = 0
        }

        if (!registers.containsKey(compareReg)) {
            registers[compareReg] = 0
        }

        val compareRegVal = registers[compareReg]!!

        val doAction =
                (operator == "==" && compareRegVal == compareValue) ||
                (operator == "!=" && compareRegVal != compareValue) ||
                (operator == ">" && compareRegVal > compareValue) ||
                (operator == "<" && compareRegVal < compareValue) ||
                (operator == ">=" && compareRegVal >= compareValue) ||
        ...
        if (doAction) {
            registers[changeReg] = registers[changeReg]!! + changeValue

            if (registers[changeReg]!! > maxInProcess) {
                maxInProcess = registers[changeReg]!!
            }
        }
    }
...
val maxInProcess = execCommands(input, registers)

println(registers.values.max())
println(maxInProcess)
