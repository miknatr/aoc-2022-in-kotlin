val input = """nyot babgr babgr kqtu kqtu kzshonp ylyk psqk
...
val total = input
        .split("\n")
        .map { line -> line.split(" ") }
        .count { words -> words.groupBy { it }.all { it.value.count() == 1 } }
...
val total2 = input
        .split("\n")
        .map { line -> line.split(" ") }
        .count { words -> words.groupBy { it.toList().sorted().joinToString("") }.all { it.value.count() == 1 } }
